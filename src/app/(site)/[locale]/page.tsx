...
import { MetricCard, PostCard, ProgramCard, ProjectCard } from '@/components/content/cards';
import { Bidi } from '@/components/ui/bidi';
import { ButtonLink, Panel, Prose, Section, SectionHeading } from '@/components/ui/primitives';
import { EmptyState } from '@/components/ui/states';
import { listMetrics, listPartners, listPosts, listPrograms, getOrganization } from '@/db/queries/content';
...
export default async function HomePage({ params }: PageProps<'/[locale]'>) {
  ...
  const [dict, org, programs, metrics, posts, projects, partners] = await Promise.all([
  ...
  const officialChannels = (org?.officialChannels ?? []).filter((c) => c.is_official);
  ...
  return (
    <>
      ...
      <div className="container-content section-gap">
        ...
        <h1 className="mbs-4 max-w-[20ch] text-h1 font-semibold text-ink md:text-[3.25rem] md:leading-[1.15]">
        ...
        <div className="mbs-10 flex flex-wrap gap-4">
          <ButtonLink href={localePath(locale, '/get-involved/partner')}>
            {dict.nav.partner}
          </ButtonLink>
      ...
      <div className="container-content">
        ...
        <Section labelledBy="home-impact">
          <SectionHeading id="home-impact" title={dict.home.impactTitle} lead={dict.home.impactLead} />
          {metrics.length === 0 ? (
            <EmptyState title={dict.states.emptyTitle} body={dict.states.emptyBody} />
          ) : (
          ...
          <div className="mbs-8">
            <Link href={localePath(locale, '/impact')}>{dict.common.viewAll}</Link>
          </div>
        </Section>
        ...
        {featuredProjects.length > 0 ? (
          <Section labelledBy="home-projects">
            ...
            <div className="mbs-8">
              <Link href={localePath(locale, '/projects')}>{dict.common.viewAll}</Link>
            </div>
          </Section>
        ) : null}
        ...
        {posts.items.length > 0 ? (
          <Section labelledBy="home-news">
            ...
            <div className="mbs-8">
              <Link href={localePath(locale, '/news')}>{dict.common.viewAll}</Link>
            </div>
          </Section>
        ) : null}
        ...
        <Section labelledBy="home-verify">
          <SectionHeading id="home-verify" title={dict.home.verifyTitle} lead={dict.home.verifyLead} />
          <Panel tone="gold">
        ...
        {partners.length > 0 ? (
          <Section labelledBy="home-partners">
            ...
            <div className="mbs-8">
              <Link href={localePath(locale, '/partners')}>{dict.common.viewAll}</Link>
            </div>
          </Section>
        ) : null}
      </div>
